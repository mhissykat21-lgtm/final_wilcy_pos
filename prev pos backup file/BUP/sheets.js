...
const USE_SHEETS = true;   // ← flip to true after you paste your URL below
const SHEETS_URL = 'https://script.google.com/macros/s/AKfycbwgRMtFU5hw9kcbIqrD0WEIMVuBy26qAgf4K6aQM_6lXMaeu9wP8KZdRO3e5YrYqBjk/exec';  // ← paste your URL here
...
async function initDB() {
  ...
  // Always load local data first so the UI is immediately usable
  // and so we never lose locally-saved sales while waiting for Sheets.
  db = loadLocal();
  ...
  try {
    ...
    // Merge: Sheets is the source of truth for items.
    // For sales, merge by id so any locally-cached sales that haven't
    // reached Sheets yet (e.g. from a previous offline session) are kept.
    const sheetsSaleIds = new Set(sheetsSales.map(s => s.id));
    const localOnlySales = db.sales.filter(s => !sheetsSaleIds.has(s.id));

    db = {
      items: sheetsItems,
      sales: [...sheetsSales, ...localOnlySales],
    };

    // If there were local-only sales that Sheets doesn't have, push them now
    if (localOnlySales.length > 0) {
      sheetsRequest('appendSales', { sales: localOnlySales }).catch(e =>
        console.warn('Failed to sync local-only sales to Sheets:', e)
      );
    }
    ...
    // Mirror merged state back to localStorage
    saveLocal();

  } catch (e) {
    console.warn('Google Sheets unavailable, falling back to localStorage:', e);
    // db is already loaded from local above — nothing more to do
...
async function saveData(opts = {}) {
  // 1. Always persist locally first (instant, no network) — do this synchronously
  //    before any await so a page unload can't lose data.
  saveLocal();
  ...
  try {
    // Run Sheets operations sequentially to avoid race conditions
    if (opts.updatedItem) {
    ...
    } else if (opts.newSales && opts.newSales.length > 0) {
